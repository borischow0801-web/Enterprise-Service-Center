...
def get_apply_detail(apply_id: int, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = GovMeetingService(db)
    return success(data=svc.get_apply_detail_admin(apply_id))


@router.post("/{apply_id}/audit", summary="审核政企约见（受理/驳回/退回补正）")
def audit_apply(apply_id: int, body: GovMeetingAuditRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = GovMeetingService(db)
...
def arrange_apply(apply_id: int, body: GovMeetingArrangeRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = GovMeetingService(db)
...
def update_arrangement(apply_id: int, arr_id: int, body: GovMeetingArrangeRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = GovMeetingService(db)
...
def confirm_apply(apply_id: int, body: GovMeetingConfirmRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = GovMeetingService(db)
...
def complete_apply(apply_id: int, body: GovMeetingCompleteRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = GovMeetingService(db)
...
def add_record(apply_id: int, body: GovMeetingRecordRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = GovMeetingService(db)
...
def finish_apply(apply_id: int, body: GovMeetingFinishRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = GovMeetingService(db)
