...
def get_booking_detail(booking_id: int, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = MeetingRoomService(db)
    return success(data=svc.get_booking_detail_admin(booking_id))


@router.post("/{booking_id}/approve", summary="审核通过")
def approve_booking(booking_id: int, body: BookingApproveRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = MeetingRoomService(db)
...
def reject_booking(booking_id: int, body: BookingRejectRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = MeetingRoomService(db)
...
def return_supplement(booking_id: int, body: BookingReturnSupplementRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = MeetingRoomService(db)
...
def complete_booking(booking_id: int, body: BookingCompleteRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = MeetingRoomService(db)
...
def no_show_booking(booking_id: int, body: BookingNoShowRequest, current: CurrentAdmin, db: Session = Depends(get_db)):
    svc = MeetingRoomService(db)
